...
and value =
  ...
  | VBool of bool
  | VName of string
  | VClo of (env * string * string * cmds) 
...
let rec findfun (l:env) (fn:value) =
  match l with
  | [] -> None
  | (fname, VClo clo) :: t -> 
        (if fname = string_of_value(fn) then Some (VClo clo)
        else findfun t fn)
  | h::t -> findfun t fn
...
let bfcallN l st =
  match st with
  | VName fname :: st -> (
    match findfun l (VName fname) with
    | None -> None
    | Some VClo (env, fn, arg, cmds) ->
              (match st with 
              | value :: st -> Some ((arg, value)::env, cmds, value, st)
              | _ -> None)
    | _ -> None)
  | VClo (env, fn, arg, cmds) :: st -> 
    (match st with 
    | value :: st -> Some ((arg, value)::env, cmds, value, st)
    | _ -> None)
  | _ -> None   

let rec findcase cases i =
  match cases with
  | [] -> None
  | (Case (Int n, cmds) :: t) -> (
    if n = i then Some(cmds)
    else findcase t i)
  | _ -> None
...
let rec eval (g : env) (l : env) (st : stack) (log : log) (cmds : cmds) :
  ...
  | IfElse (cmds1, cmds2) :: cmds -> (
    ...
    | _ -> (g, log, None))
  | Fun (fname, arg, fcmds) :: cmds -> (
        let rec fclo = VClo (((fname, fclo)::l), fname, arg, fcmds) in
          eval g ((fname, fclo)::l) st log cmds)
  | Call :: cmds -> (
    match callN g l st log with 
    | Some (g, log, st) -> eval g l st log cmds
    | None -> (g, log, None))
  | Try tcmds :: cmds -> (
    match tryN g l st log tcmds with
    | Some (newg, newlog, newst) -> eval newg l newst newlog cmds
    | None -> (g, log, None)) 
  | Switch cases :: cmds -> (
    match st with
    | VInt i :: st -> (
      match findcase cases i with
      | None -> (g, log, None)
      | Some(scmds) -> eval g l st log (List.append scmds cmds))
    | _ -> (g, log, None)
  )
...
and callN g l st log= 
    match bfcallN l st with
    | Some (newl, cmds, v, stack) -> 
      (match eval g newl [] log cmds with
      |(_, log, Some st) -> 
        (match st with
        |[] -> None
        |h::t -> Some (g, log, h::stack)) 
      | _ -> None)
    | _ -> None

and tryN g l stack log tcmds =
    match eval g l [] log tcmds with
    | (newg, newlog, Some st) -> (
      match st with 
      |[] -> None
      |h::t -> Some(newg, newlog, h::stack))
    | (newg, newlog, None) -> Some(newg, newlog, stack)

(* and switchN g l st log cases =
    match st with
    | VInt i :: st -> (
      match findcase cases i with
      | None -> None
      | Some(cmds) ->
        match eval g l st log cmds with)
    | _ -> None *)
...
and cmd_parser () =
  ...
  <|> begin_parser () <|> ifelse_parser () <|> fun_parser () <|> call_parser ()
  <|> try_parser () <|> switch_parser ()
